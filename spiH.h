...
int open_device(char *dev_path);  //打开一个spi
...
int transfer_readData(int comNum, int length,
		struct RS422_data* RS422_data_buff); //读取串口length长度的字节数到receiveData中
...
void Spi2proces(struct RS422_data* RS422_MIRROR_buff,
		struct RS422_data* RS422_VCODE_buff,
		struct RS422_data* RS422_BAK1_buff);
...
typedef void (*CB_Readbyte)();
void Set_CB_Read( CB_Readbyte func);
